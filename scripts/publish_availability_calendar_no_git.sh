...
if grep -q 'data-availability-calendar' "$SITE/index.html"; then
  CALENDAR_PAGE="$SITE/index.html"
elif [[ -s "$SITE/booking.html" ]] &&
     grep -q 'data-availability-calendar' "$SITE/booking.html"; then
  CALENDAR_PAGE="$SITE/booking.html"
else
  fail "Calendar markup missing from index.html and booking.html"
fi

echo "Calendar page: $CALENDAR_PAGE"
...
BOOKING_HTML="$(curl -sS -L "https://$DOMAIN/booking.html")"

echo "$BOOKING_HTML" |
  grep -q 'data-availability-calendar' ||
  fail "Live calendar validation failed on booking.html"
